...
FROM            (SELECT        cp_conciliacion_Caja_det.IdEmpresa, cp_conciliacion_Caja_det.IdConciliacion_Caja, cp_conciliacion_Caja_det.Secuencia, cp_conciliacion_Caja_det.IdEmpresa_OGiro, cp_conciliacion_Caja_det.IdTipoCbte_Ogiro, 
                                                    ...
                                                    cp_orden_giro.co_total - isnull(ret.valor_retencion, 0) AS valor_a_pagar, cp_conciliacion_Caja_det.Valor_a_aplicar, cp_orden_giro.co_observacion, cp_conciliacion_Caja.Saldo_cont_al_periodo, 
                                                    cp_conciliacion_Caja.Ingresos, abs(cp_conciliacion_Caja.Total_fact_vale) Total_fact_vale, cp_conciliacion_Caja.Dif_x_pagar_o_cobrar, 'FACTURA' AS TIPO, cp_conciliacion_Caja.Fecha_ini, 
                                                    cp_conciliacion_Caja.Fecha_fin, op.Valor_a_pagar valor_a_reponer, caj_caja.ca_Descripcion NombreCaja
                          FROM            cp_conciliacion_Caja INNER JOIN
                                                    ...
                                                    cp_orden_giro.IdTipoCbte_Ogiro = ret.IdTipoCbte_Ogiro AND cp_orden_giro.IdCbteCble_Ogiro = ret.IdCbteCble_Ogiro LEFT JOIN
                                                        (SELECT        IdEmpresa, IdOrdenPago, Valor_a_pagar
                                                          FROM            cp_orden_pago_det) op ON op.IdEmpresa = cp_conciliacion_Caja.IdEmpresa AND op.IdOrdenPago = cp_conciliacion_Caja.IdOrdenPago_op
														  inner join caj_Caja on cp_conciliacion_Caja.IdEmpresa = caj_Caja.IdEmpresa and cp_conciliacion_Caja.IdCaja = caj_Caja.IdCaja
                          UNION ALL
                          SELECT        cp_conciliacion_Caja_det_x_ValeCaja.IdEmpresa, cp_conciliacion_Caja_det_x_ValeCaja.IdConciliacion_Caja, cp_conciliacion_Caja_det_x_ValeCaja.Secuencia, 
                                                   cp_conciliacion_Caja_det_x_ValeCaja.IdEmpresa_movcaja, cp_conciliacion_Caja_det_x_ValeCaja.IdTipocbte_movcaja, cp_conciliacion_Caja_det_x_ValeCaja.IdCbteCble_movcaja, NULL AS co_factura, 
                                                   tb_persona.pe_nombreCompleto, caj_Caja_Movimiento.cm_fecha, caj_Caja_Movimiento.cm_valor, 0 AS valor_retencion, caj_Caja_Movimiento.cm_valor AS valor_a_pagar, 
                                                   caj_Caja_Movimiento.cm_valor AS valor_a_aplicar, caj_Caja_Movimiento.cm_observacion, cp_conciliacion_Caja.Saldo_cont_al_periodo, cp_conciliacion_Caja.Ingresos, abs(cp_conciliacion_Caja.Total_fact_vale) 
                                                   Total_fact_vale, cp_conciliacion_Caja.Dif_x_pagar_o_cobrar, 'VALE' AS TIPO, cp_conciliacion_Caja.Fecha_ini, cp_conciliacion_Caja.Fecha_fin, op.Valor_a_pagar valor_a_reponer, caj_Caja.ca_Descripcion AS NombreCaja
                          FROM            cp_conciliacion_Caja INNER JOIN
                                                   ...
                                                   tb_persona ON caj_Caja_Movimiento.IdPersona = tb_persona.IdPersona LEFT JOIN
                                                       (SELECT        IdEmpresa, IdOrdenPago, Valor_a_pagar
                                                         FROM            cp_orden_pago_det) op ON op.IdEmpresa = cp_conciliacion_Caja.IdEmpresa AND op.IdOrdenPago = cp_conciliacion_Caja.IdOrdenPago_op
														  inner join caj_Caja on cp_conciliacion_Caja.IdEmpresa = caj_Caja.IdEmpresa and cp_conciliacion_Caja.IdCaja = caj_Caja.IdCaja
														 ) A
