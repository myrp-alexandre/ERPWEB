...
FROM     (SELECT cp_conciliacion_Caja_det.IdEmpresa, cp_conciliacion_Caja_det.IdConciliacion_Caja, cp_conciliacion_Caja_det.Secuencia, cp_conciliacion_Caja_det.IdEmpresa_OGiro, cp_conciliacion_Caja_det.IdTipoCbte_Ogiro, 
                                    ...
                                    caj_caja.ca_Descripcion NombreCaja, '' AS tm_descripcion, cp_conciliacion_Caja.IdUsuarioCreacion, u.Nombre NombreUsuario
                  FROM      cp_conciliacion_Caja INNER JOIN
                                    ...
                                    cp_orden_giro.IdTipoCbte_Ogiro = ret.IdTipoCbte_Ogiro AND cp_orden_giro.IdCbteCble_Ogiro = ret.IdCbteCble_Ogiro LEFT JOIN
                                        (SELECT IdEmpresa, IdOrdenPago, Valor_a_pagar
                                         FROM      cp_orden_pago_det) op ON op.IdEmpresa = cp_conciliacion_Caja.IdEmpresa AND op.IdOrdenPago = cp_conciliacion_Caja.IdOrdenPago_op INNER JOIN
                                    caj_Caja ON cp_conciliacion_Caja.IdEmpresa = caj_Caja.IdEmpresa AND cp_conciliacion_Caja.IdCaja = caj_Caja.IdCaja LEFT JOIN seg_usuario AS U
									ON U.IdUsuario = cp_conciliacion_Caja.IdUsuarioCreacion
                  UNION ALL
                  SELECT cp_conciliacion_Caja_det_x_ValeCaja.IdEmpresa, cp_conciliacion_Caja_det_x_ValeCaja.IdConciliacion_Caja, cp_conciliacion_Caja_det_x_ValeCaja.Secuencia, cp_conciliacion_Caja_det_x_ValeCaja.IdEmpresa_movcaja, 
                                    ...
                                    cp_conciliacion_Caja.Fecha_ini, cp_conciliacion_Caja.Fecha_fin, op.Valor_a_pagar valor_a_reponer, caj_Caja.ca_Descripcion AS NombreCaja, caj_Caja_Movimiento_Tipo.tm_descripcion,
									cp_conciliacion_Caja.IdUsuarioCreacion, u.Nombre NombreUsuario
                  FROM     cp_conciliacion_Caja INNER JOIN
                                    ...
                                    ct_cbtecble ON caj_Caja_Movimiento.IdEmpresa = ct_cbtecble.IdEmpresa AND caj_Caja_Movimiento.IdTipocbte = ct_cbtecble.IdTipoCbte AND caj_Caja_Movimiento.IdCbteCble = ct_cbtecble.IdCbteCble LEFT JOIN
                                    caj_Caja_Movimiento_Tipo ON caj_Caja_Movimiento_Tipo.IdEmpresa = caj_Caja_Movimiento.IdEmpresa AND caj_Caja_Movimiento_Tipo.IdTipoMovi = caj_Caja_Movimiento.IdTipoMovi INNER JOIN
