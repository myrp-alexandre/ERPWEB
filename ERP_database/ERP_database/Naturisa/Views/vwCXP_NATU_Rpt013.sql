...
--OG
						...
						where --cxp.IdEmpresa = @IdEmpresa and cxp.IdProveedor between @IdProveedor_ini and @IdProveedor_fin and cp_proveedor.IdClaseProveedor between @IdClaseProveedor_ini and @IdClaseProveedor_fin and 									
									exists(
									...
									--and cxp.co_FechaFactura between @Fecha_ini and @Fecha_fin 
									AND CXP.Estado = 'A'
				UNION
				--ND
						...
						WHERE        (cxp.DebCre = 'D') AND (det.dc_Valor < 0) and exists(
									...
									--and cxp.cn_fecha between @Fecha_ini and @Fecha_fin 
									AND CXP.Estado = 'A'
				UNION
				--OP
						...
						WHERE        (cp_orden_pago.IdTipo_Persona = 'PROVEE') AND (NOT EXISTS
								...
								--and cp_orden_pago.Fecha between @Fecha_ini and @Fecha_fin 
								AND cp_orden_pago.Estado = 'A'
				union
				--RETENCION
						...
						FROM            ct_plancta AS ct_plancta_det INNER JOIN
										...
										inner join tb_persona as per on per.IdPersona = cp_proveedor.IdPersona
									where --cp_orden_giro.IdEmpresa = @IdEmpresa and cp_orden_giro.IdProveedor between @IdProveedor_ini and @IdProveedor_fin and cp_proveedor.IdClaseProveedor between @IdClaseProveedor_ini and @IdClaseProveedor_fin and 
									exists(
									select clase.IdEmpresa from cp_proveedor_clase clase
									where clase.IdEmpresa = det.IdEmpresa
									and clase.IdCtaCble_CXP = det.IdCtaCble
									)
									--and cp_retencion.fecha between @Fecha_ini and @Fecha_fin 
									AND cp_retencion.Estado = 'A'
				UNION
				--NC
							...
							FROM            ct_cbtecble_det AS det INNER JOIN
											...
											ct_cbtecble_tipo ON cab.IdEmpresa = ct_cbtecble_tipo.IdEmpresa AND cab.IdTipoCbte = ct_cbtecble_tipo.IdTipoCbte
											inner join tb_persona as per on per.IdPersona = cp_proveedor.IdPersona
							where --cp_nota_DebCre.IdEmpresa = @IdEmpresa and cp_nota_DebCre.IdProveedor between @IdProveedor_ini and @IdProveedor_fin and cp_proveedor.IdClaseProveedor between @IdClaseProveedor_ini and @IdClaseProveedor_fin and 
							cp_nota_DebCre.DebCre = 'C' AND cp_nota_DebCre.Estado ='A' and exists(
										select clase.IdEmpresa from cp_proveedor_clase clase
										where clase.IdEmpresa = det.IdEmpresa
										and clase.IdCtaCble_CXP = det.IdCtaCble
										) 
										--and cp_nota_DebCre.cn_fecha between @Fecha_ini and @Fecha_fin 
										and cp_nota_DebCre.Estado = 'A' and det.dc_Valor > 0
				UNION
				--CONCILIACION OG
				...
				FROM            cp_orden_pago INNER JOIN
								...
								inner join tb_persona as per on per.IdPersona = cp_proveedor.IdPersona
				WHERE        (cp_orden_pago.IdTipo_Persona = 'PROVEE') 
				--AND (cp_orden_pago.IdEmpresa = @IdEmpresa) AND (cp_orden_pago.IdEntidad between @IdProveedor_ini and @IdProveedor_fin) and cp_proveedor.IdClaseProveedor between @IdClaseProveedor_ini and @IdClaseProveedor_fin
					--		 AND (ba_Cbte_Ban.cb_Fecha BETWEEN @Fecha_ini AND @Fecha_fin) 
					AND (cp_orden_pago.Estado = 'A') AND EXISTS
									(SELECT        IdEmpresa
									FROM            cp_proveedor_clase AS clase
