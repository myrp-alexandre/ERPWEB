...
AS
SELECT        re.IdEmpresa, re.IdRetencion, re.IdEmpresa_Ogiro, Og.IdCbteCble_Ogiro AS IdCbte_CXP, Og.IdTipoCbte_Ogiro, SUBSTRING(TiDoc.Descripcion, 1, 3) + '-' + + Og.co_serie + '-' + Og.co_factura AS Factura_Prov, 
                         Og.co_FechaFactura, 'RT-' + re.serie1 + '-' + re.serie2 + '-' + RTRIM(CAST(re.NumRetencion AS varchar(20))) AS NumeroRT, re.fecha AS Fecha_RT, 
                         ...
                         re.re_EstaImpresa AS ImpresaRT, re.observacion AS observacionRT, re.re_Tiene_RTiva, re.re_Tiene_RFuente, Prov.IdProveedor, per.pe_nombreCompleto AS Nombre_Prov, TiDoc.Descripcion, 
                         SUM(dbo.cp_retencion_det.re_valor_retencion) AS Total_Retencion, re.serie1 + '-' + re.serie2 AS serie, re.NumRetencion, re.IdUsuario, re.Fecha_Transac, re.Fecha_Autorizacion, re.CodDocumentoTipo, re.serie1, re.serie2, 
                         Og.co_serie
FROM            dbo.cp_retencion_det INNER JOIN
                         ...
                         re.serie2 = dbo.tb_sis_Documento_Tipo_Talonario.PuntoEmision AND re.serie1 = dbo.tb_sis_Documento_Tipo_Talonario.Establecimiento AND 
                         re.NumRetencion = dbo.tb_sis_Documento_Tipo_Talonario.NumDocumento LEFT OUTER JOIN
                         ...
                         dbo.cp_TipoDocumento AS TiDoc ON Og.IdOrden_giro_Tipo = TiDoc.CodTipoDocumento ON re.IdEmpresa_Ogiro = Og.IdEmpresa AND re.IdCbteCble_Ogiro = Og.IdCbteCble_Ogiro AND 
                         re.IdTipoCbte_Ogiro = Og.IdTipoCbte_Ogiro INNER JOIN
...
GROUP BY re.IdEmpresa_Ogiro, Og.IdCbteCble_Ogiro, Og.IdTipoCbte_Ogiro, SUBSTRING(TiDoc.Descripcion, 1, 3) + '-' + + Og.co_serie + '-' + Og.co_factura, Og.co_FechaFactura, re.IdRetencion, 
                         ...
                         re.NAutorizacion, re.Estado, re.re_EstaImpresa, re.observacion, re.re_Tiene_RTiva, re.re_Tiene_RFuente, Prov.IdProveedor, per.pe_nombreCompleto, TiDoc.Descripcion, re.IdEmpresa, re.serie1 + '-' + re.serie2, 
                         re.NumRetencion, re.IdUsuario, re.Fecha_Transac, re.Fecha_Autorizacion, re.CodDocumentoTipo, re.serie1, re.serie2, Og.co_serie
GO
      ...
      Begin Tables = 
         ...
         Begin Table = "per"
            Begin Extent = 
               ...
               Right = 286
            End
            Di', @level0type = N'SCHEMA', @level0name = N'dbo', @level1type = N'VIEW', @level1name = N'vwcp_retencion';
